<#
.SYNOPSIS
  KorgenKassa -- standalone Windows uninstaller.

.DESCRIPTION
  Removes the KorgenKassa app, its shortcuts, and its registry uninstall entry.
  The Keep/Delete decision is made before KorgenKassa is closed so an active
...
.EXAMPLE
  Download and run directly, no need to clone the repo:
    irm https://github.com/FreeOpenSourcePOS/KorgenKassa/releases/latest/download/uninstall-windows.ps1 -OutFile uninstall-windows.ps1
...
$script:AppName = 'KorgenKassa'
$script:AppProcessName = 'KorgenKassa'
...
function Invoke-Removal($path, $description) {
  ...
  Write-Warn 'make sure KorgenKassa is completely closed (check Task Manager for "KorgenKassa.exe", it may be hiding in the system tray) and re-run this script.'
...
function Get-FloProcesses {
  try {
    ...
    Mark-Partial ("could not inspect running KorgenKassa processes: {0}" -f $_.Exception.Message)
    return $null
  }
}

function Confirm-KorgenKassaStopped {
  if ($script:DryRun) { return $true }
  $activeProcesses = @(Get-FloProcesses)
  if ($script:ProcessInspectionFailed) {
    Mark-Partial 'could not verify that KorgenKassa is stopped; skipping destructive cleanup to protect active writes'
    return $false
  }
  if ($activeProcesses.Count -gt 0) {
    Mark-Partial 'KorgenKassa is still running; skipping destructive cleanup to protect active writes'
    return $false
  }
  return $true
}
...
function Confirm-NoActiveUninstallWork {
  return (Confirm-KorgenKassaStopped) -and (Confirm-ChildUninstallerStopped)
}
...
function Close-KorgenKassa {
  ...
  Write-Warn ("KorgenKassa did not close gracefully within {0} seconds; forcing it to close." -f $script:AppGracefulCloseTimeoutSeconds)
  ...
  foreach ($process in $remaining) {
    try {
      ...
      Mark-Partial ("could not force-close KorgenKassa process {0}: {1}" -f $process.Id, $_.Exception.Message)
    }
  }
  ...
  Mark-Partial ("could not confirm that KorgenKassa closed after the {0}-second force wait; locked files may remain" -f $script:AppForceCloseTimeoutSeconds)
  return $false
}
...
function Invoke-RegistryRemoval($entry) {
  ...
  if ([string]::IsNullOrWhiteSpace($registryPath)) {
    Mark-Partial 'could not determine the registry path for the KorgenKassa uninstall entry'
    return $false
  }
...
function Test-FloUninstallerIsTrusted {
  ...
  # KorgenKassa: it must be named like an NSIS uninstaller and live either under a
...
function Invoke-KorgenKassaUninstall {
  param(
    [switch]$PurgeData,
    [switch]$DryRun
  )
  ...
  Write-Step 'KorgenKassa uninstaller (Windows)'
  ...
  Write-Step 'Closing KorgenKassa if it is running...'
  $appClosed = [bool](Close-KorgenKassa)
  if (-not $appClosed -and -not $script:DryRun) {
    Write-Warn 'KorgenKassa did not close completely; destructive cleanup will be skipped while it is running.'
  }
  ...
  if ($appEntries.Count -gt 0) {
    foreach ($entry in $appEntries) {
      ...
      if ($uninstallerExe -and $uninstallerExists -and -not (Test-FloUninstallerIsTrusted -UninstallerExe $uninstallerExe -InstallLocation $entryInstallLocation)) {
        Write-Warn "refusing to run an unverified executable at `"$uninstallerExe`" -- it is not a known KorgenKassa install location; falling back to manual cleanup"
      } elseif ($uninstallerExe -and $uninstallerExists) {
        Write-Step "Running the app's own uninstaller silently..."
        # /S first, then whatever came from the registry (e.g. /D=C:\Program Files\KorgenKassa):
        ...
        if ($script:DryRun) {
          ...
          Write-Warn "skipping the app's own uninstaller because KorgenKassa or its child uninstaller is still running"
        }
  ...
  if (-not $destructiveCleanupAllowed) {
    Write-Warn 'Skipping app files, registry, and business-data removal until KorgenKassa is completely closed.'
  }
  ...
  if ($destructiveCleanupAllowed) {
    ...
    # to the known KorgenKassa install roots below.
    if (-not [string]::IsNullOrWhiteSpace([string]$env:LOCALAPPDATA)) {
    ...
    Write-Log 'keeping your data because destructive cleanup was skipped while KorgenKassa was running'
  }
  ...
  if ($script:DryRun) {
    ...
    Write-Warn 'Some KorgenKassa files, processes, or registry entries could not be removed or verified.'
    Write-Warn 'Close KorgenKassa completely and re-run this script to finish cleanup.'
  }
...
if ($MyInvocation.InvocationName -ne '.') {
  $result = Invoke-KorgenKassaUninstall -PurgeData:$PurgeData -DryRun:$DryRun
