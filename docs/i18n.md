...
| Language | Code | BCP-47 locale | Endonym | Direction |
| --- | --- | --- | --- | --- |
| English (canonical) | `en` | `en` | English | `ltr` |
| Russian | `ru` | `ru-RU` | Русский | `ltr` |
| Kazakh | `kk` | `kk-KZ` | Қазақша | `ltr` |

[`frontend/src/lib/i18n/languages.ts`](../frontend/src/lib/i18n/languages.ts) is the source of truth for this table. Staff switch the UI language from the language switcher in the **app sidebar**, from **Settings → Store Settings → Language**, or during the **Setup wizard**; all three switchers read the same registry and offer only `selectable: true` entries.
...
If you are correcting typos, refining wording, or improving existing translations (such as Russian `ru.json` or Kazakh `kk.json`):
...
- Russian: [`frontend/src/lib/i18n/messages/ru.json`](../frontend/src/lib/i18n/messages/ru.json)
- Kazakh: [`frontend/src/lib/i18n/messages/kk.json`](../frontend/src/lib/i18n/messages/kk.json)
...
Run the zero-dependency scaffolding helper using the lowercase 2- or 3-letter ISO 639 language code (e.g. `uz` for Uzbek, `tr` for Turkish, `ar` for Arabic). FloCafe currently ships `en`, `ru`, and `kk`, so the code must be one that does not exist yet:

```sh
npm run i18n:add -- ar
```

This creates `frontend/src/lib/i18n/messages/ar.json` containing the canonical structure copied from `en.json`. The script uses atomic creation (`COPYFILE_EXCL`) and will never overwrite an existing file.
...
#### Field reference:
- `locale`: Default BCP-47 locale tag used for number/currency/date formatting defaults (e.g. `'en'`, `'ru-RU'`, `'kk-KZ'`).
- `nativeName`: Endonym displayed in the language switcher (e.g. `'Русский'`, `'Қазақша'`).
- `direction`: Text direction (`'ltr'` or `'rtl'`). Every shipped language is currently `'ltr'`; see the RTL section below before adding an `'rtl'` entry.
- `selectable`: Set `true` to offer it in the sidebar, Settings, and Setup language switchers. Set `false` if the translation is in progress and undergoing review.
...
npm run dev
```
1. Open the language switcher in the **app sidebar**, or navigate to **Settings → Store Settings → Language** (or the Setup Wizard at `/setup`). All three switchers are driven by the same registry, so a `selectable: true` entry appears in each of them.
...
```sh
git checkout -b feat/i18n-add-arabic
git add frontend/src/lib/i18n/messages/ar.json frontend/src/lib/i18n/languages.ts
git commit -m "feat(i18n): add Arabic language translation"
```
...
No RTL language ships today — `en`, `ru`, and `kk` are all `direction: 'ltr'`. The RTL *infrastructure* is deliberately retained: the direction plumbing in the registry and providers, the `.rtl-flip` utility, the `<Ltr>` component, and the CSS logical-property conventions all remain in place, and the `test:rtl-*` suites still enforce them across the UI. Follow the conventions below so that adding a bidirectional or RTL language (such as Arabic `ar`, Hebrew `he`, Persian `fa`, or Urdu `ur`) remains a translation task rather than a layout rewrite.

### 1. Document direction synchronization

When a language declaring `direction: 'rtl'` is selected:
...
FloCafe includes automated RTL verification test suites. They assert the conventions above against the source tree, so they remain meaningful even while no RTL language is shipped:
```sh
...
All three shipped languages (`en`, `ru`, `kk`) are currently registered with `selectable: true`, so the repository contains no staged-but-hidden translation.
...
- **UI Language:** Controls the text strings in the interface (English, Russian, and Kazakh) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
...
Translating FloCafe into Japanese or Arabic does not require creating a tax pack, and adding a tax pack for a new country does not require adding a new UI language.
